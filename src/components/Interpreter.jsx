...
export default function Interpreter(props) {
    ...
    const apiKey = import.meta.env.VITE_MISTRALAI_API_KEY
    ...
    useEffect(() => {
        const interpretData = async () => {
            ...
            try {
                setIsCooldown(true);
                setTimeout(() => setIsCooldown(false), 300000); // 5 min cooldown

                const client = new Mistral({ apiKey: apiKey });

                console.log('client', client)
                console.log('ai weatherdata', weatherData)
    ...
    return (
        <div className="h-56 w-1/4 absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-2 p-3 rounded-xl bg-black/30">
            <h2 className="flex flex-row items-center text-3xl text-white">
            ...
            {!summary ? (
                <p className="text-lg text-white animate-pulse">Generating Summary...</p>
            ) : (
                <p className="text-lg text-white">{summary}</p>
            )}
            <p className="absolute bottom-2 text-grey">Powered by <a href="https://mistral.ai/">Mistral AI</a></p>
        </div>
    );
};